...
from JMetal.custom_algorithm.custom_nsga import GeneticAlgorithm
...
from JMetal.custom_algorithm.custom_nsga import NSGAII_custom, SPEA2
...
class Find_Optimum_Allocation(IntegerProblem):
    ...
    def __init__(self, number_of_variables: int = 2, number_of_city=7, demand_list=[]):
        ...
        self.number_of_constraints = number_of_city
        ...
        if len(self.min_allocation_list) != self.number_of_variables:
            random_ind = random.randint(0, self.number_of_city - 1)
            self.min_allocation_list[random_ind] = self.min_allocation_list[random_ind] + 1
    ...
    def evaluate(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        self.evaluate_constraint(solution)
        return solution

    def get_name(self) -> str:
        return 'Doctor Allocation'

    def evaluate_constraint(self, solution: IntegerSolution) -> IntegerSolution:
        for i in range(self.number_of_city):
            allocation_in_city = len(list(filter(lambda x: x == i, solution.variables)))
            solution.constraints[i] = -1 * (math.fabs(self.min_allocation_list[i] - allocation_in_city)/(self.number_of_variables - self.min_allocation_list[i]))
...
class Optimize_Allocation():
    ...
    def allocate(self):
        algorithm = GeneticAlgorithm(
            ...
            termination_criterion=StoppingByEvaluations(max_evaluations=300000)
        )
...
if __name__ == "__main__":
    ...
    for i in range(5):
        params = (
            (i + iter_start), mutation_probability, crossover_type, 36, [6, 10, 8, 10, 3, 4, 8], 7, mutation_type,
            300000,
            0.9)
